...
import torch
import pickle
from FunnyNodules.models import ModelXtoCtoY
from FunnyNodules.dataset import create_FunnyNodule
def train_X_to_C_to_y(args, dataset_kwargs, model_kwargs):

    dataloaders, dataset_sizes = create_FunnyNodule(train_size=1800, val_size=200, test_size=500, img_size=224, **dataset_kwargs)

    # ---- Model fitting ----
    model = ModelXtoCtoY(model_kwargs)
    results = model.fit(dataloaders=dataloaders, dataset_sizes=dataset_sizes)

    # analyse_contrastivity
    model.analyse_FunnyNodules()
...
def save_model_results(model, results, args, dataset_kwargs, model_kwargs, exp_name=None):
    ...
    # Save code
    src_models_code_dir = os.path.join(base_dir, 'FunnyNodules')
    dest_code_dir = os.path.join(base_dir, unique_name, 'code')
    ...
    config_path = os.path.join(base_dir, unique_name, 'config.pkl')
    pickle.dump(config, open(config_path, 'wb'))

    # Save results
    results_path = os.path.join(base_dir, unique_name, 'results.pkl')
    pickle.dump(results, open(results_path, 'wb'))

    # Save model weights.
    weights_path = os.path.join(base_dir, unique_name, 'model_weights.pth')
    try:
        print('Model saved A')
...
def generate_configs(args):
    ...
    concepts_to_use = ['roundness', 'spiculation', 'edge_sharpness', 'size_attr', 'intensity', 'internal_structure']
    ...
    CLASSES_PER_COLUMN = {'roundness': 1, 'spiculation': 1, 'edge_sharpness': 1, 'size_attr': 1, 'intensity': 1, 'internal_structure': 1,'mal': 5}
...
def parse_arguments(experiment):
    ...
    parser.add_argument('--eval_model', type=str, choices=['X_to_C_to_y'])
