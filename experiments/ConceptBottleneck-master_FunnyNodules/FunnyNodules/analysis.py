...
def assess_performance(y, yhat, names, prediction_type, prefix, verbose=False):
    ...
    metrics = {}
    print("in assessperformance in analysis")
    print(y.shape)
    print(yhat.shape)
    if y.shape[1] == 1:
        y_int = np.round(y[:, 0]).astype(int)
        yhat_diff = np.abs(yhat[:, 0] - y_int)
        within_1 = np.mean(yhat_diff <= 1)
        metrics[prefix + '_' + names[0] + '_within_1_score'] = within_1
    if y.shape[1] == 6:
        final_ranges = [4, 4, 4, 4, 4, 1]
        for i, (name, max_val) in enumerate(zip(names, final_ranges)):
            y_scaled = y[:, i] * max_val
            yhat_scaled = yhat[:, i] * max_val
            y_rounded = np.round(y_scaled).astype(int)
            yhat_rounded = np.round(yhat_scaled).astype(int)
            if max_val==1:
                within_1 = np.mean(np.abs(yhat_rounded - y_rounded) == 0)
            else:
                within_1 = np.mean(np.abs(yhat_rounded - y_rounded) <= 1)
            metric_key = f"{prefix}_{name}_within_1_score"
    ...
    for metric in metrics:
        # print(metric, metrics[metric], type(metrics[metric]))
        ...
        #     print(f"⚠️ {name} is non-finite: {metrics[metric]}")
